...
extern const coap_endpoint_t app_endpoints[];

#ifdef DEBUG
void coap_dumpHeader(coap_header_t *hdr)
{
    os_printf(LM_WIFI, LL_INFO, "Header:\n");
    os_printf(LM_WIFI, LL_INFO, "  ver  0x%02X\n", hdr->ver);
    os_printf(LM_WIFI, LL_INFO, "  t    0x%02X\n", hdr->t);
    os_printf(LM_WIFI, LL_INFO, "  tkl  0x%02X\n", hdr->tkl);
    os_printf(LM_WIFI, LL_INFO, "  code 0x%02X\n", hdr->code);
    os_printf(LM_WIFI, LL_INFO, "  id   0x%02X%02X\n", hdr->id[0], hdr->id[1]);
}
#endif
...
void coap_dump(const uint8_t *buf, size_t buflen, bool bare)
{
    ...
    {
        while(buflen--)
            os_printf(LM_WIFI, LL_INFO, "%02X%s", *buf++, (buflen > 0) ? " " : "");
    }
    else
    {
        os_printf(LM_WIFI, LL_INFO, "Dump: ");
        while(buflen--)
            os_printf(LM_WIFI, LL_INFO, "%c", *buf++);
        os_printf(LM_WIFI, LL_INFO, "\n");
    }
}
#endif
...
int coap_parseOption(coap_option_t *option, uint16_t *running_delta, const uint8_t **buf, size_t buflen)
{
    ...
    //os_printf(LM_WIFI, LL_INFO, "option num=%d\n", delta + *running_delta);
...
{
    size_t i;
    os_printf(LM_WIFI, LL_INFO, " Options:\n");
    for (i=0;i<numopt;i++)
    {
        os_printf(LM_WIFI, LL_INFO, "  0x%02X [ ", opts[i].num);
...
void coap_dumpPacket(coap_packet_t *pkt)
{
    ...
    os_printf(LM_WIFI, LL_INFO, "Payload: ");
    coap_dump(pkt->payload.p, pkt->payload.len, false);
    os_printf(LM_WIFI, LL_INFO, "\n");
}
#endif
