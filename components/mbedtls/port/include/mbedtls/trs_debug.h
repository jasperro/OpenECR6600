...
#ifndef _TRS_DEBUG_H_
#define _TRS_DEBUG_H_
...
/** @brief Enable mbedTLS debug logging via the trs_log mechanism.
 *
 * mbedTLS internal debugging is filtered from a specified mbedTLS
 * threshold level to trs_log level at runtime:
 *
 * - 1 - Warning
 * - 2 - Info
 * - 3 - Debug
 ...
 *
 * This function will set the trs log level for "mbedtls" to the specified mbedTLS
 * threshold level that matches. However, the overall max trs log level must be set high
...
/** @brief Disable mbedTLS debug logging via the trs_log mechanism.
 *
 */
