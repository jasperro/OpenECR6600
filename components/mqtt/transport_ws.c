...
static int ws_connect(transport_handle_t t, const char *host, int port, int timeout_ms)
{
    ...
    if (transport_connect(ws->parent, host, port, timeout_ms) < 0) {
        os_printf(LM_APP, LL_INFO, "Error connect to ther server");
    }
    ...
    int len =   snprintf(ws->buffer, DEFAULT_WS_BUFFER,
                         ...
                         host, port,
                         client_key);
    os_printf(LM_APP, LL_INFO, "Write upgrate request\r\n%s", ws->buffer);
    if (transport_write(ws->parent, ws->buffer, len, timeout_ms) <= 0) {
        os_printf(LM_APP, LL_INFO, "Error write Upgrade header %s", ws->buffer);
        return -1;
    }
    if ((len = transport_read(ws->parent, ws->buffer, DEFAULT_WS_BUFFER, timeout_ms)) <= 0) {
        os_printf(LM_APP, LL_INFO, "Error read response for Upgrade header %s", ws->buffer);
        return -1;
    }
    char *server_key = get_http_header(ws->buffer, "Sec-WebSocket-Accept:");
    if (server_key == NULL) {
        os_printf(LM_APP, LL_INFO, "Sec-WebSocket-Accept not found");
        return -1;
    }

    unsigned char client_key_b64[128] = {0};
    unsigned char valid_client_key[20] = {0};
    ...
    accept_key[outlen] = 0;
    os_printf(LM_APP, LL_INFO, "server key=%s, send_key=%s, accept_key=%s", (char *)server_key, (char*)client_key, accept_key);
    if (strcmp((char*)accept_key, (char*)server_key) != 0) {
        os_printf(LM_APP, LL_INFO, "Invalid websocket key");
        return -1;
    }
    return 0;
}
...
static int ws_write(transport_handle_t t, const char *buff, int len, int timeout_ms)
{
    ...
    if (transport_write(ws->parent, ws_header, header_len, timeout_ms) != header_len) {
        os_printf(LM_APP, LL_INFO, "Error write header");
        return -1;
    }
...
static int ws_read(transport_handle_t t, char *buffer, int len, int timeout_ms)
{
    ...
    if ((rlen = transport_read(ws->parent, buffer, len, timeout_ms)) <= 0) {
        os_printf(LM_APP, LL_INFO, "Error read data");
        return rlen;
    }
    ...
    data_ptr++;
    os_printf(LM_APP, LL_INFO, "Opcode: %d, mask: %d, len: %d\r\n", opcode, mask, payload_len);
    if (payload_len == 126) {
        // headerLen += 2;
