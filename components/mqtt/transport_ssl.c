...
static int ssl_connect(transport_handle_t t, const char *host, int port, int timeout_ms)
{
    ...
    if ((ret = mbedtls_ssl_config_defaults(&ssl->conf,
        ...
        os_printf(LM_CMD, LL_INFO, "mbedtls_ssl_config_defaults returned %d", ret);
        goto exit;
    }

    if ((ret = mbedtls_ctr_drbg_seed(&ssl->ctr_drbg, mbedtls_entropy_func, &ssl->entropy, NULL, 0)) != 0) {
        os_printf(LM_APP, LL_INFO, "mbedtls_ctr_drbg_seed returned %d", ret);
        goto exit;
    }
    ...
    if (ssl->cert_pem_data) {
        ...
        if ((ret = mbedtls_ssl_set_hostname(&ssl->ctx, host)) != 0) {
            os_printf(LM_APP, LL_INFO, "mbedtls_ssl_set_hostname returned -0x%x", -ret);
            goto exit;
        }
    } else {
    ...
    if (ssl->client_cert_pem_data && ssl->client_key_pem_data) {
        ...
        os_printf(LM_APP, LL_INFO, "You have to provide both client_cert_pem and client_key_pem for mutual authentication");
        goto exit;
    }
    ...
    os_printf(LM_APP, LL_INFO, "Connect to %s:%d", host, port);
    char port_str[8] = {0};
    sprintf(port_str, "%d", port);
    if ((ret = mbedtls_net_connect(&ssl->client_fd, host, port_str, MBEDTLS_NET_PROTO_TCP)) != 0) {
        os_printf(LM_APP, LL_INFO, "mbedtls_net_connect returned -%x", -ret);
        goto exit;
    }
    ...
    if((ret = mbedtls_ssl_set_hostname(&ssl->ctx, host)) != 0) {
        os_printf(LM_APP, LL_INFO, " failed\n  ! mbedtls_ssl_set_hostname returned %d\n\n", ret);
        goto exit;
    }

    os_printf(LM_APP, LL_INFO, "Performing the SSL/TLS handshake...");

    while ((ret = mbedtls_ssl_handshake(&ssl->ctx)) != 0) {
        if (ret != MBEDTLS_ERR_SSL_WANT_READ && ret != MBEDTLS_ERR_SSL_WANT_WRITE) {
            os_printf(LM_APP, LL_INFO, "mbedtls_ssl_handshake returned -0x%x", -ret);
            goto exit;
        }
    }

    os_printf(LM_APP, LL_INFO, "Verifying peer X.509 certificate...");
    ...
    os_printf(LM_APP, LL_INFO, "Cipher suite is %s", mbedtls_ssl_get_ciphersuite(&ssl->ctx));
    return ret;
exit:
    ssl_close(t);
    return ret;
...
static int ssl_write(transport_handle_t t, const char *buffer, int len, int timeout_ms)
{
    ...
    if ((poll = transport_poll_write(t, timeout_ms)) <= 0) {
        os_printf(LM_APP, LL_INFO, "Poll timeout or error, errno=%s, fd=%d, timeout_ms=%d", strerror(errno), ssl->client_fd.fd, timeout_ms);
        return poll;
    }
    ret = mbedtls_ssl_write(&ssl->ctx, (const unsigned char *) buffer, len);
    if (ret <= 0) {
        os_printf(LM_APP, LL_INFO, "mbedtls_ssl_write error, errno=%s", strerror(errno));
    }
    return ret;
}
...
static int ssl_close(transport_handle_t t)
{
    ...
    if (ssl->ssl_initialized) {
        os_printf(LM_APP, LL_INFO, "Cleanup mbedtls");
