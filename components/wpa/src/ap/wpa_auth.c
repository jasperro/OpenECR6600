...
static void wpa_rekey_gmk(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (random_get_bytes(wpa_auth->group->GMK, WPA_GMK_LEN)) {
		wpa_printf_error(MSG_ERROR, "Failed to get random data for WPA "
			   "initialization.");
	} else {
				    ...
				    struct wpa_auth_callbacks *cb)
{
	...
	if (wpa_auth_gen_wpa_ie(wpa_auth)) {
		wpa_printf_error(MSG_ERROR, "Could not generate WPA IE.");
		os_free(wpa_auth);
		return NULL;
	}
	...
	if (wpa_auth->pmksa == NULL) {
		wpa_printf_error(MSG_ERROR, "PMKSA cache initialization failed.");
	...
	if (wpa_auth->ft_pmk_cache == NULL) {
		wpa_printf_error(MSG_ERROR, "FT PMK cache initialization failed.");
		os_free(wpa_auth->group);
		os_free(wpa_auth->wpa_ie);
...
SM_STATE(WPA_PTK, AUTHENTICATION2)
{
	...
	if (random_get_bytes(sm->ANonce, WPA_NONCE_LEN)) {
		wpa_printf_error(MSG_ERROR, "WPA: Failed to get random data for "
			   "ANonce.");
		sm->Disconnect = TRUE;
		return;
	}
...
SM_STATE(WPA_PTK, PTKINITNEGOTIATING)
{
	...
	if (wpa_key_mgmt_ft(sm->wpa_key_mgmt)) {
		...
		if (res < 0) {
			wpa_printf_error(MSG_ERROR, "FT: Failed to insert "
	...
	if (wpa_key_mgmt_ft(sm->wpa_key_mgmt)) {
		...
		if (res < 0) {
			wpa_printf_error(MSG_ERROR, "FT: Failed to insert FTIE "
...
static int wpa_sm_step(struct wpa_state_machine *sm)
{
	...
	if (sm->in_step_loop) {
		 ...
		 * recursive call. */
		wpa_printf_error(MSG_ERROR, "WPA: wpa_sm_step() called recursively");
		return 0;
	}
