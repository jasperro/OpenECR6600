...
int hostapd_setup_wpa(struct hostapd_data *hapd)
{
	...
	if (hapd->wpa_auth == NULL) {
		wpa_printf_error(MSG_ERROR, "WPA initialization failed.");
		return -1;
	}

	if (hostapd_set_privacy(hapd, 1)) {
		wpa_printf_error(MSG_ERROR, "Could not set PrivacyInvoked "
	...
	if (rsn_preauth_iface_init(hapd)) {
		wpa_printf_error(MSG_ERROR, "Initialization of RSN "
	...
	if (!hostapd_drv_none(hapd) &&
		...
		if (hapd->l2 == NULL &&
			...
			wpa_printf_error(MSG_ERROR, "Failed to open l2_packet "
				   "interface");
			return -1;
		}
	}
