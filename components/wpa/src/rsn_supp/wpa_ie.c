			      ...
			      int key_mgmt)
{
	...
	if (suite == 0) {
		wpa_printf_warning(MSG_WARNING, "Invalid group cipher (%d).",
			   group_cipher);
		return -1;
	}
	...
	*pos++ = 0;
	...
	if (suite == 0 ||
		...
		wpa_printf_warning(MSG_WARNING, "Invalid pairwise cipher (%d).",
			   pairwise_cipher);
		return -1;
	}
	RSN_SELECTOR_PUT(pos, suite);
	...
	*pos++ = 0;
	if (key_mgmt == WPA_KEY_MGMT_IEEE8021X) {
		...
		wpa_printf_warning(MSG_WARNING, "Invalid key management type (%d).",
			   key_mgmt);
		return -1;
	}
	pos += WPA_SELECTOR_LEN;
