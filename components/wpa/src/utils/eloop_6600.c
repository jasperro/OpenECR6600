...
int eloop_init(void)
{
	...
	if (eloop.epollfd < 0) {
		wpa_printf_error(MSG_ERROR, "%s: epoll_create1 failed. %s",
	...
	if (eloop.kqueuefd < 0) {
		wpa_printf_error(MSG_ERROR, "%s: kqueue failed: %s",
			   __func__, strerror(errno));
		return -1;
	}
#endif /* CONFIG_ELOOP_KQUEUE */
...
static int eloop_sock_queue(int sock, eloop_event_type type)
{
	...
	if (epoll_ctl(eloop.epollfd, EPOLL_CTL_ADD, sock, &ev) < 0) {
		wpa_printf_error(MSG_ERROR, "%s: epoll_ctl(ADD) for fd=%d failed: %s",
...
static int eloop_sock_queue(int sock, eloop_event_type type)
{
	...
	if (kevent(eloop.kqueuefd, &ke, 1, NULL, 0, NULL) == -1) {
		wpa_printf_error(MSG_ERROR, "%s: kevent(ADD) for fd=%d failed: %s",
			   __func__, sock, strerror(errno));
		return -1;
	}
	return 0;
                                     ...
                                     void *eloop_data, void *user_data)
{
	...
	if (eloop.count + 1 > eloop.epoll_max_event_num) {
		...
		if (temp_events == NULL) {
			wpa_printf_error(MSG_ERROR, "%s: malloc for epoll failed: %s",
				   __func__, strerror(errno));
			return -1;
		}
                                         ...
                                         int sock)
{
	...
	if (epoll_ctl(eloop.epollfd, EPOLL_CTL_DEL, sock, NULL) < 0) {
		wpa_printf_error(MSG_ERROR, "%s: epoll_ctl(DEL) for fd=%d failed: %s",
			   __func__, sock, strerror(errno));
		return;
	}
	...
	if (kevent(eloop.kqueuefd, &ke, 1, NULL, 0, NULL) < 0) {
		wpa_printf_error(MSG_ERROR, "%s: kevent(DEL) for fd=%d failed: %s",
			   __func__, sock, strerror(errno));
		return;
	}
	os_memset(&eloop.fd_table[sock], 0, sizeof(struct eloop_sock));
...
{
	wpa_printf_error(MSG_ERROR, "eloop: could not process SIGINT or SIGTERM in "
...
void eloop_run(void)
{
	...
	os_printf(LM_APP, LL_INFO, "eloop_run select again 1 \n");
		...
		if (res < 0 && errno != EINTR && errno != 0) {
			wpa_printf_error(MSG_ERROR, "eloop: %s: %s",
#ifdef CONFIG_ELOOP_POLL
				   "poll"
#endif /* CONFIG_ELOOP_POLL */
#ifdef CONFIG_ELOOP_SELECT
