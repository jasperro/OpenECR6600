			  ...
			  const char *name, const char *val)
{
	...
	if (os_strncmp(pos, "be_", 3) == 0) {
		...
		wpa_printf_error(MSG_ERROR, "Unknown WMM name '%s'", pos);
		return -1;
	}
	...
	if (os_strcmp(pos, "aifs") == 0) {
		v = atoi(val);
		if (v < 1 || v > 255) {
			wpa_printf_error(MSG_ERROR, "Invalid AIFS value %d", v);
			return -1;
		}
		ac->aifs = v;
		...
		v = atoi(val);
		if (v < 0 || v > 15) {
			wpa_printf_error(MSG_ERROR, "Invalid cwMin value %d", v);
			return -1;
		}
		ac->cwmin = v;
		...
		v = atoi(val);
		if (v < 0 || v > 15) {
			wpa_printf_error(MSG_ERROR, "Invalid cwMax value %d", v);
			return -1;
		}
		ac->cwmax = v;
		...
		v = atoi(val);
		if (v < 0 || v > 0xffff) {
			wpa_printf_error(MSG_ERROR, "Invalid txop value %d", v);
			return -1;
		}
		ac->txop_limit = v;
		...
		v = atoi(val);
		if (v < 0 || v > 1) {
			wpa_printf_error(MSG_ERROR, "Invalid acm value %d", v);
			return -1;
		}
		ac->admission_control_mandatory = v;
	} else {
		wpa_printf_error(MSG_ERROR, "Unknown wmm_ac_ field '%s'", pos);
		return -1;
	}
