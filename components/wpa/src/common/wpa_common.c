	       ...
	       const u8 *ric, size_t ric_len, u8 *mic)
{
	...
	if (kck_len != 16) {
		wpa_printf_warning(MSG_WARNING, "FT: Unsupported KCK length %u",
...
int wpa_insert_pmkid(u8 *ies, size_t *ies_len, const u8 *pmkid)
{
	...
	if (start >= end) {
		wpa_printf_error(MSG_ERROR, "FT: Could not find RSN IE in "
			   "IEs data");
		return -1;
	}
	...
	if (rpos == rend) {
		...
		*rpos++ = 0;
		...
		rpos += 2;
		if (rpos > rend) {
			wpa_printf_error(MSG_ERROR, "FT: Could not parse RSN IE in "
				   "IEs data");
			return -1;
		}
	}
