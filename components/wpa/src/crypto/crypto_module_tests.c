...
static int test_siv(void)
{
	...
	if (aes_siv_encrypt(key, plaintext, sizeof(plaintext),
			    1, addr, len, out)) {
		wpa_printf_error(MSG_ERROR, "AES-SIV mode encryption failed");
		return 1;
	}
	...
	if (aes_siv_decrypt(key, iv_c, sizeof(iv_c), 1, addr, len, out)) {
		wpa_printf_error(MSG_ERROR, "AES-SIV mode decryption failed");
		return 1;
	}
			     ...
			     unsigned int i)
{
	...
	if (omac1_aes_128(tv->k, tv->msg, tv->msg_len, result) ||
		...
		wpa_printf_error(MSG_ERROR, "OMAC1-AES-128 test vector %u failed", i);
		return 1;
	}
	...
	if (omac1_aes_128(key, msg, sizeof(msg), result) ||
		...
		wpa_printf_error(MSG_ERROR, "OMAC1-AES-128 short test mismatch");
		return 1;
	}
...
static int test_eax(void)
{
	...
	if (aes_128_eax_encrypt(key, nonce, sizeof(nonce), hdr, sizeof(hdr),
		...
		wpa_printf_error(MSG_ERROR, "AES-128 EAX mode encryption failed");
		return 1;
	}
	...
	if (aes_128_eax_decrypt(key, nonce, sizeof(nonce), hdr, sizeof(hdr),
				data, sizeof(data), tag)) {
		wpa_printf_error(MSG_ERROR, "AES-128 EAX mode decryption failed");
		return 1;
	}
...
static int test_cbc(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(vectors); i++) {
		...
		if (aes_128_cbc_encrypt(tv->key, tv->iv, buf, tv->len) ||
			...
			wpa_printf_error(MSG_ERROR, "AES-CBC encrypt %d failed", i);
			ret++;
		}
		...
		if (aes_128_cbc_decrypt(tv->key, tv->iv, buf, tv->len) ||
			...
			wpa_printf_error(MSG_ERROR, "AES-CBC decrypt %d failed", i);
			ret++;
		}
...
static int test_ecb(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(vectors); i++) {
		...
		if (hexstr2bin(tv->key, key, sizeof(key)) ||
			...
			wpa_printf_error(MSG_ERROR, "Invalid AES-ECB test vector %u",
				   i);
			ret++;
			continue;
		}
		...
		if (aes_128_encrypt_block(key, plain, out) < 0 ||
			...
			wpa_printf_error(MSG_ERROR, "AES-ECB encrypt %u failed", i);
			ret++;
		}
	}
...
static int test_key_wrap(void)
{
	...
	if (aes_wrap(kek41, sizeof(kek41), sizeof(plain41) / 8, plain41,
		     result)) {
		wpa_printf_error(MSG_ERROR, "AES-WRAP-128 reported failure");
		ret++;
	}
	if (os_memcmp(result, crypt41, sizeof(crypt41)) != 0) {
		wpa_printf_error(MSG_ERROR, "AES-WRAP-128 failed");
		ret++;
	}
	if (aes_unwrap(kek41, sizeof(kek41), sizeof(plain41) / 8, crypt41,
		       result)) {
		wpa_printf_error(MSG_ERROR, "AES-UNWRAP-128 reported failure");
		ret++;
	}
	if (os_memcmp(result, plain41, sizeof(plain41)) != 0) {
		wpa_printf_error(MSG_ERROR, "AES-UNWRAP-128 failed");
		ret++;
	}
	...
	if (aes_wrap(kek42, sizeof(kek42), sizeof(plain42) / 8, plain42,
		     result)) {
		wpa_printf_error(MSG_ERROR, "AES-WRAP-192 reported failure");
		ret++;
	}
	if (os_memcmp(result, crypt42, sizeof(crypt42)) != 0) {
		wpa_printf_error(MSG_ERROR, "AES-WRAP-192 failed");
		ret++;
	}
	if (aes_unwrap(kek42, sizeof(kek42), sizeof(plain42) / 8, crypt42,
		       result)) {
		wpa_printf_error(MSG_ERROR, "AES-UNWRAP-192 reported failure");
		ret++;
	}
	if (os_memcmp(result, plain42, sizeof(plain42)) != 0) {
		wpa_printf_error(MSG_ERROR, "AES-UNWRAP-192 failed");
		ret++;
	}
#endif /* CONFIG_BORINGSSL */
	...
	if (aes_wrap(kek43, sizeof(kek43), sizeof(plain43) / 8, plain43,
		     result)) {
		wpa_printf_error(MSG_ERROR, "AES-WRAP-256 reported failure");
		ret++;
	}
	if (os_memcmp(result, crypt43, sizeof(crypt43)) != 0) {
		wpa_printf_error(MSG_ERROR, "AES-WRAP-256 failed");
		ret++;
	}
	if (aes_unwrap(kek43, sizeof(kek43), sizeof(plain43) / 8, crypt43,
		       result)) {
		wpa_printf_error(MSG_ERROR, "AES-UNWRAP-256 reported failure");
		ret++;
	}
	if (os_memcmp(result, plain43, sizeof(plain43)) != 0) {
		wpa_printf_error(MSG_ERROR, "AES-UNWRAP-256 failed");
		ret++;
	}
...
static int test_ms_funcs(void)
{
	...
	if (nt_password_hash((u8 *) password, os_strlen(password), buf) ||
		...
		wpa_printf_error(MSG_ERROR, "nt_password_hash failed");
		errors++;
	}
	...
	if (generate_nt_response(auth_challenge, peer_challenge,
		...
		wpa_printf_error(MSG_ERROR, "generate_nt_response failed");
		errors++;
	}
	...
	if (hash_nt_password_hash(password_hash, buf) ||
		...
		wpa_printf_error(MSG_ERROR, "hash_nt_password_hash failed");
		errors++;
	}
	...
	if (generate_authenticator_response((u8 *) password,
		...
		wpa_printf_error(MSG_ERROR, "generate_authenticator_response failed");
		errors++;
	}

	if (get_master_key(password_hash_hash, nt_response, buf) ||
	    os_memcmp(master_key, buf, sizeof(master_key)) != 0) {
		wpa_printf_error(MSG_ERROR, "get_master_key failed");
		errors++;
	}
	...
	if (get_asymetric_start_key(master_key, buf, sizeof(send_start_key),
		...
		wpa_printf_error(MSG_ERROR, "get_asymetric_start_key failed");
		errors++;
	}

	if (errors)
		wpa_printf_error(MSG_ERROR, "ms_funcs: %d errors", errors);
	else
