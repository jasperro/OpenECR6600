...
int random_get_bytes(void *buf, size_t len)
{
	...
	while (left) {
		...
		if (crypto_get_random(tmp, sizeof(tmp)) < 0) {
			wpa_printf_error(MSG_ERROR, "random: No entropy available "
...
int random_pool_ready(void)
{
	...
	if (fd < 0) {
		wpa_printf_error(MSG_ERROR, "random: Cannot open /dev/random: %s",
			   strerror(errno));
		return -1;
	}
	...
	if (res < 0) {
		wpa_printf_error(MSG_ERROR, "random: Cannot read from /dev/random: "
			   "%s", strerror(errno));
		res = 0;
	}
...
static void random_write_entropy(void)
{
	...
	if (f == NULL) {
		wpa_printf_error(MSG_ERROR, "random: Could not open entropy file %s "
	...
	fclose(f);
	if (fail) {
		wpa_printf_error(MSG_ERROR, "random: Could not write entropy data "
