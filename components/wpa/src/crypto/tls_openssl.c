...
static int tls_add_ca_from_keystore(X509_STORE *ctx, const char *key_alias)
{
	...
	if (!stack) {
		wpa_printf_warning(MSG_WARNING, "TLS: Failed to parse certificate: %s",
			   key_alias);
		return -1;
	}
					    ...
					    const char *encoded_key_alias)
{
	...
	if (len & 1) {
		wpa_printf_warning(MSG_WARNING, "Invalid hex-encoded alias: %s",
			   encoded_key_alias);
		return rc;
	}
...
static int tls_cryptoapi_cert(SSL *ssl, const char *name)
{
	...
	if (priv == NULL || rsa_meth == NULL) {
		wpa_printf_warning(MSG_WARNING, "CryptoAPI: Failed to allocate memory "
	...
	if (tls_openssl_ref_count == 0) {
		...
		if (conf && conf->fips_mode) {
			...
			if (!fips_enabled && !FIPS_mode_set(1)) {
				wpa_printf_error(MSG_ERROR, "Failed to enable FIPS "
					   "mode");
...
#else /* OPENSSL_FIPS */
		if (conf && conf->fips_mode) {
			wpa_printf_error(MSG_ERROR, "FIPS mode requested, but not "
				   "supported");
			os_free(tls_global);
			tls_global = NULL;
			return NULL;
...
#error "This code depends on OPENSSL_NO_ENGINE being defined by BoringSSL."
	...
	int ret = -1;
	if (engine_id == NULL) {
		wpa_printf_error(MSG_ERROR, "ENGINE: Engine ID not set");
		return -1;
	}
	...
	if (!conn->engine) {
		wpa_printf_error(MSG_ERROR, "ENGINE: engine %s not available [%s]",
			   engine_id, ERR_error_string(ERR_get_error(), NULL));
		goto err;
	}
	if (ENGINE_init(conn->engine) != 1) {
		wpa_printf_error(MSG_ERROR, "ENGINE: engine init failed "
	...
	if (pin && ENGINE_ctrl_cmd_string(conn->engine, "PIN", pin, 0) == 0) {
		wpa_printf_error(MSG_ERROR, "ENGINE: cannot set pin [%s]",
			   ERR_error_string(ERR_get_error(), NULL));
		goto err;
	}
#endif
	...
	if (cert_id || ca_cert_id) {
		...
		if (!ENGINE_ctrl(conn->engine, ENGINE_CTRL_GET_CMD_FROM_NAME,
			...
			wpa_printf_error(MSG_ERROR, "ENGINE: engine does not support"
		       ...
		       const void *buf, size_t len, SSL *ssl, void *arg)
{
	...
	if (content_type == 24 && len >= 3 && pos[0] == 1) {
		...
		if (payload_len + 3 > len) {
			wpa_printf_error(MSG_ERROR, "OpenSSL: Heartbeat attack detected");
...
static int tls_verify_cb(int preverify_ok, X509_STORE_CTX *x509_ctx)
{
	...
	if (!preverify_ok) {
		wpa_printf_warning(MSG_WARNING, "TLS: Certificate verification failed,"
	...
	if (depth == 0 && match && os_strstr(buf, match) == NULL) {
		wpa_printf_warning(MSG_WARNING, "TLS: Subject '%s' did not "
		...
		wpa_printf_warning(MSG_WARNING, "TLS: altSubjectName match "
		...
		wpa_printf_warning(MSG_WARNING, "TLS: Domain suffix match '%s' not found",
			   suffix_match);
		preverify_ok = 0;
		openssl_tls_fail_event(conn, err_cert, err, depth, buf,
		...
		wpa_printf_warning(MSG_WARNING, "TLS: Domain match '%s' not found",
			   domain_match);
		preverify_ok = 0;
		openssl_tls_fail_event(conn, err_cert, err, depth, buf,
				       "Domain mismatch",
			       ...
			       X509 **cert)
{
	...
	if (!ENGINE_ctrl_cmd(conn->engine, "LOAD_CERT_CTRL",
		...
		wpa_printf_error(MSG_ERROR, "ENGINE: cannot load client cert with id"
			   " '%s' [%s]", cert_id,
			   ERR_error_string(err, NULL));
		if (tls_is_pin_error(err))
	...
	if (!params.cert) {
		wpa_printf_error(MSG_ERROR, "ENGINE: did not properly cert with id"
			   " '%s'", cert_id);
		return TLS_SET_PARAMS_ENGINE_PRV_INIT_FAILED;
	}
	*cert = params.cert;
...
static int tls_connection_engine_private_key(struct tls_connection *conn)
{
	...
	return 0;
#else /* OPENSSL_NO_ENGINE */
	wpa_printf_error(MSG_ERROR, "SSL: Configuration uses engine, but "
...
static int tls_connection_dh(struct tls_connection *conn, const char *dh_file)
{
	...
	if (dh_file == NULL)
		return 0;
	wpa_printf_error(MSG_ERROR, "TLS: openssl does not include DH support, but "
		   "dh_file specified");
	return -1;
#else /* OPENSSL_NO_DH */
	DH *dh;
