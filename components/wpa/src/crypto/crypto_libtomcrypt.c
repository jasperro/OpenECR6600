...
struct crypto_public_key * crypto_public_key_import(const u8 *key, size_t len)
{
	...
	if (res != CRYPT_OK) {
		wpa_printf_error(MSG_ERROR, "LibTomCrypt: Failed to import "
			   "public key (res=%d '%s')",
	...
	if (pk->rsa.type != PK_PUBLIC) {
		wpa_printf_error(MSG_ERROR, "LibTomCrypt: Public key was not of "
			   "correct type");
		rsa_free(&pk->rsa);
		os_free(pk);
		return NULL;
						      ...
						      const char *passwd)
{
	...
	if (pk->rsa.type != PK_PRIVATE) {
		wpa_printf_error(MSG_ERROR, "LibTomCrypt: Private key was not of "
			   "correct type");
		rsa_free(&pk->rsa);
		os_free(pk);
		return NULL;
...
int crypto_global_init(void)
{
	...
	if (register_hash(&md4_desc) < 0 ||
		...
		wpa_printf_error(MSG_ERROR, "TLSv1: Failed to register "
