				 ...
				 const u8 *addr[], const size_t *len, u8 *mac)
{
	...
	if (!EVP_DigestInit_ex(ctx, type, NULL)) {
		wpa_printf_error(MSG_ERROR, "OpenSSL: EVP_DigestInit_ex failed: %s",
	...
	for (i = 0; i < num_elem; i++) {
		if (!EVP_DigestUpdate(ctx, addr[i], len[i])) {
			wpa_printf_error(MSG_ERROR, "OpenSSL: EVP_DigestUpdate "
				   "failed: %s",
				   ERR_error_string(ERR_get_error(), NULL));
			EVP_MD_CTX_free(ctx);
			return -1;
	...
	if (!EVP_DigestFinal(ctx, mac, &mac_len)) {
		wpa_printf_error(MSG_ERROR, "OpenSSL: EVP_DigestFinal failed: %s",
			   ERR_error_string(ERR_get_error(), NULL));
		EVP_MD_CTX_free(ctx);
		return -1;
	}
...
void aes_encrypt(void *ctx, const u8 *plain, u8 *crypt)
{
	...
	if (EVP_EncryptUpdate(c, crypt, &clen, plain, 16) != 1) {
		wpa_printf_error(MSG_ERROR, "OpenSSL: EVP_EncryptUpdate failed: %s",
			   ERR_error_string(ERR_get_error(), NULL));
	}
}
...
void aes_encrypt_deinit(void *ctx)
{
	...
	if (EVP_EncryptFinal_ex(c, buf, &len) != 1) {
		wpa_printf_error(MSG_ERROR, "OpenSSL: EVP_EncryptFinal_ex failed: "
			   "%s", ERR_error_string(ERR_get_error(), NULL));
	}
	if (len != 0) {
		wpa_printf_error(MSG_ERROR, "OpenSSL: Unexpected padding length %d "
			   "in AES encrypt", len);
	}
...
void aes_decrypt(void *ctx, const u8 *crypt, u8 *plain)
{
	...
	if (EVP_DecryptUpdate(c, plain, &plen, crypt, 16) != 1) {
		wpa_printf_error(MSG_ERROR, "OpenSSL: EVP_DecryptUpdate failed: %s",
			   ERR_error_string(ERR_get_error(), NULL));
	}
}
...
void aes_decrypt_deinit(void *ctx)
{
	...
	if (EVP_DecryptFinal_ex(c, buf, &len) != 1) {
		wpa_printf_error(MSG_ERROR, "OpenSSL: EVP_DecryptFinal_ex failed: "
			   "%s", ERR_error_string(ERR_get_error(), NULL));
	}
	if (len != 0) {
		wpa_printf_error(MSG_ERROR, "OpenSSL: Unexpected padding length %d "
