				...
				int line, const char *value)
{
	...
	if (tmp == NULL) {
		wpa_printf_error(MSG_ERROR, "Line %d: failed to parse %s '%s'.",
			   line, data->name,
	...
	if (data->param3 && res_len < (size_t) data->param3) {
		wpa_printf_error(MSG_ERROR, "Line %d: too short %s (len=%lu "
	...
	if (data->param4 && res_len > (size_t) data->param4) {
		wpa_printf_error(MSG_ERROR, "Line %d: too long %s (len=%lu "
				...
				int line, const char *value)
{
	...
	if (*end) {
		wpa_printf_error(MSG_ERROR, "Line %d: invalid number \"%s\"",
			   line, value);
		return -1;
	}
	...
	*dst = val;
	...
	if (data->param3 && *dst < (long) data->param3) {
		wpa_printf_error(MSG_ERROR, "Line %d: too small %s (value=%d "
	...
	if (data->param4 && *dst > (long) data->param4) {
		wpa_printf_error(MSG_ERROR, "Line %d: too large %s (value=%d "
				  ...
				  const char *value)
{
	...
	if (hwaddr_aton(value, ssid->bssid)) {
		wpa_printf_error(MSG_ERROR, "Line %d: Invalid BSSID '%s'.",
			   line, value);
		return -1;
	}
	ssid->bssid_set = 1;
				...
				const char *value)
{
	...
	if (*value == '"') {
		...
		if (pos)
			...
			len = os_strlen(value);
		if (len < 8 || len > 63) {
			wpa_printf_error(MSG_ERROR, "Line %d: Invalid passphrase "
		...
		wpa_printf_error(MSG_ERROR, "Line %d: ASCII passphrase not "
			   "supported.", line);
		return -1;
	...
	if (hexstr2bin(value, ssid->psk, PMK_LEN) ||
	    value[PMK_LEN * 2] != '\0') {
		wpa_printf_error(MSG_ERROR, "Line %d: Invalid PSK '%s'.",
			   line, value);
		return -1;
	}
	...
	while (*start != '\0') {
		...
		*end = '\0';
		if (os_strcmp(start, "WPA") == 0)
			...
			wpa_printf_error(MSG_ERROR, "Line %d: invalid proto '%s'",
				   line, start);
			errors++;
		}
	...
	while (*start != '\0') {
		...
		*end = '\0';
		if (os_strcmp(start, "WPA-PSK") == 0)
			...
			wpa_printf_error(MSG_ERROR, "Line %d: invalid key_mgmt '%s'",
				   line, start);
			errors++;
		}
...
static int wpa_config_parse_cipher(int line, const char *value)
{
	...
	if (val < 0) {
		wpa_printf_error(MSG_ERROR, "Line %d: invalid cipher '%s'.",
			   line, value);
		return -1;
	}
	if (val == 0) {
		wpa_printf_error(MSG_ERROR, "Line %d: no cipher values configured.",
			   line);
		return -1;
	}
	return val;
				     ...
				     const char *value)
{
	...
	if (val & ~WPA_ALLOWED_PAIRWISE_CIPHERS) {
		wpa_printf_error(MSG_ERROR, "Line %d: not allowed pairwise cipher "
			   "(0x%x).", line, val);
		return -1;
	}
				  ...
				  const char *value)
{
	...
	if (val & ~WPA_ALLOWED_GROUP_CIPHERS) {
		wpa_printf_error(MSG_ERROR, "Line %d: not allowed group cipher "
			   "(0x%x).", line, val);
		return -1;
	}
	...
	while (*start != '\0') {
		...
		*end = '\0';
		if (os_strcmp(start, "OPEN") == 0)
			...
			wpa_printf_error(MSG_ERROR, "Line %d: invalid auth_alg '%s'",
				   line, start);
			errors++;
		}
	...
	while (*start != '\0') {
		...
		*end = '\0';
		...
		if (methods[num_methods].vendor == EAP_VENDOR_IETF &&
			...
			wpa_printf_error(MSG_ERROR, "Line %d: unknown EAP method "
				   "'%s'", line, start);
			wpa_printf_error(MSG_ERROR, "You may need to add support for"
				     ...
				     const char *value)
{
	...
	if (os_strncmp(value, "hash:", 5) != 0) {
		...
		tmp = wpa_config_parse_string(value, &res_len);
		if (tmp == NULL) {
			wpa_printf_error(MSG_ERROR, "Line %d: failed to parse "
				   "password.", line);
			return -1;
		}
		wpa_hexdump_ascii_key(MSG_MSGDUMP, data->name,
	...
	if (os_strlen(value + 5) != 2 * 16) {
		wpa_printf_error(MSG_ERROR, "Line %d: Invalid password hash length "
	...
	if (hexstr2bin(value + 5, hash, 16)) {
		os_free(hash);
		wpa_printf_error(MSG_ERROR, "Line %d: Invalid password hash", line);
		return -1;
	}
				    ...
				    const char *value, int idx)
{
	...
	if (buf == NULL) {
		wpa_printf_error(MSG_ERROR, "Line %d: Invalid WEP key %d '%s'.",
			   line, idx, value);
		return -1;
	}
	if (*len > MAX_WEP_KEY_LEN) {
		wpa_printf_error(MSG_ERROR, "Line %d: Too long WEP key %d '%s'.",
			   line, idx, value);
		os_free(buf);
		return -1;
	}
	if (*len && *len != 5 && *len != 13 && *len != 16) {
		wpa_printf_error(MSG_ERROR, "Line %d: Invalid WEP key length %u - "
					    ...
					    const char *value)
{
	...
	if (hwaddr_aton(value, ssid->go_p2p_dev_addr)) {
		wpa_printf_error(MSG_ERROR, "Line %d: Invalid GO P2P Device Address '%s'.",
			   line, value);
		return -1;
	}
	ssid->bssid_set = 1;
				     ...
				     const char *value)
{
	...
	if (hwaddr_aton(pos, p->addr)) {
		wpa_printf_error(MSG_ERROR, "Line %d: Invalid psk_list address '%s'",
			   line, pos);
		os_free(p);
		return -1;
	}
	pos += 17;
	if (*pos != '-') {
		wpa_printf_error(MSG_ERROR, "Line %d: Invalid psk_list '%s'",
			   line, pos);
		os_free(p);
		return -1;
	}
	...
	if (hexstr2bin(pos, p->psk, PMK_LEN) || pos[PMK_LEN * 2] != '\0') {
		wpa_printf_error(MSG_ERROR, "Line %d: Invalid psk_list PSK '%s'",
			   line, pos);
		os_free(p);
		return -1;
	}
					     ...
					     const char *value)
{
	...
	if (rates == NULL) {
		wpa_printf_error(MSG_ERROR, "Line %d: Invalid mesh_basic_rates '%s'",
			   line, value);
		return -1;
	}
	if (rates[0] == 0) {
		   ...
		   int line)
{
	...
	for (i = 0; i < NUM_SSID_FIELDS; i++) {
		...
		if (ret < 0) {
			if (line) {
				wpa_printf_error(MSG_ERROR, "Line %d: failed to "
	...
	if (i == NUM_SSID_FIELDS) {
		if (line) {
			wpa_printf_error(MSG_ERROR, "Line %d: unknown network field "
				   "'%s'.", line, var);
		}
		ret = -1;
	}
				       ...
				       const char *pos)
{
	...
	if (has_newline(pos)) {
		wpa_printf_error(MSG_ERROR, "Line %d: invalid %s value with newline",
			   line, data->name);
		return -1;
	}
				     ...
				     const char *pos)
{
	...
	if (tmp == NULL) {
		wpa_printf_error(MSG_ERROR, "Line %d: failed to parse %s",
			   line, data->name);
		return -1;
	}
				   ...
				   const char *pos)
{
	...
	if (uuid_str2bin(pos, config->uuid)) {
		wpa_printf_error(MSG_ERROR, "Line %d: invalid UUID", line);
		return -1;
	}
					 ...
					 const char *pos)
{
	if (hexstr2bin(pos, config->os_version, 4)) {
		wpa_printf_error(MSG_ERROR, "Line %d: invalid os_version", line);
		return -1;
	}
	...
	struct wpa_config *config, int line, const char *pos)
{
	...
	if (!len) {
		wpa_printf_error(MSG_ERROR, "Line %d: "
	...
	if (tmp) {
		...
		wpa_printf_error(MSG_ERROR, "Can not allocate "
	...
	struct wpa_config *config, int line, const char *pos)
{
	...
	if (config->num_sec_device_types >= MAX_SEC_DEVICE_TYPES) {
		wpa_printf_error(MSG_ERROR, "Line %d: too many sec_device_type "
			   "items", line);
		return -1;
	}
...
fail:
	os_free(pref);
	wpa_printf_error(MSG_ERROR, "Line %d: Invalid p2p_pref_chan list", line);
	return -1;
}
	...
	struct wpa_config *config, int line, const char *pos)
{
	...
	if (ret < 0) {
		wpa_printf_error(MSG_ERROR, "Line %d: Invalid p2p_no_go_freq", line);
		return -1;
	}
	...
	struct wpa_config *config, int line, const char *pos)
{
	...
	if (hwaddr_aton2(pos, config->hessid) < 0) {
		wpa_printf_error(MSG_ERROR, "Line %d: Invalid hessid '%s'",
			   line, pos);
		return -1;
	}
#endif
	...
	struct wpa_config *config, int line, const char *pos)
{
	...
	if (groups == NULL) {
		wpa_printf_error(MSG_ERROR, "Line %d: Invalid sae_groups '%s'",
			   line, pos);
		return -1;
	}
	...
	struct wpa_config *config, int line, const char *pos)
{
	...
	if (!len) {
		wpa_printf_error(MSG_ERROR, "Line %d: invalid ap_vendor_elements",
			   line);
		return -1;
	}
	...
	if (tmp) {
		...
		if (hexstr2bin(pos, p, len)) {
			wpa_printf_error(MSG_ERROR, "Line %d: invalid "
		...
		wpa_printf_error(MSG_ERROR, "Cannot allocate memory for "
			   "ap_vendor_elements");
		return -1;
	}
	...
	if (i == NUM_GLOBAL_FIELDS) {
		...
		if (os_strncmp(pos, "wmm_ac_", 7) == 0) {
			...
			if (tmp == NULL) {
				if (line < 0)
					return -1;
				wpa_printf_error(MSG_ERROR, "Line %d: invalid line "
					   "'%s'", line, pos);
				return -1;
			}
			*tmp++ = '\0';
			/*if (hostapd_config_wmm_ac(config->wmm_ac_params, pos,
						  tmp)) {
				wpa_printf_error(MSG_ERROR, "Line %d: invalid WMM "
					   "AC item", line);
				return -1;
			}*/
		}
		...
		if (line < 0)
			return -1;
		wpa_printf_error(MSG_ERROR, "Line %d: unknown global field '%s'.",
			   line, pos);
		ret = -1;
	}
