					 ...
					 const u8 *ies, size_t ies_len)
{
	...
	if (action != 1) {
		wpa_printf_error(MSG_ERROR, "Unsupported send_ft_action action %d",
			   action);
		return -1;
	}
					       ...
					       const char **txt)
{
	...
	*txt = default_txt;
	...
	if (*txt == NULL) {
		wpa_printf_warning(MSG_WARNING, "No message for request %d", field);
		ret = NULL;
	}
					    ...
					    const char *default_txt)
{
	...
	field_name = wpa_supplicant_ctrl_req_to_string(field, default_txt,
						       &txt);
	if (field_name == NULL) {
		wpa_printf_warning(MSG_WARNING, "Unhandled EAP param %d needed",
			   field);
		return;
	}
...
int wpa_supplicant_init_wpa(struct wpa_supplicant *wpa_s)
{
	...
	if (ctx == NULL) {
		wpa_printf_error(MSG_ERROR, "Failed to allocate WPA context.");
		return -1;
	}
	...
	if (wpa_s->wpa == NULL) {
		wpa_printf_error(MSG_ERROR, "Failed to initialize WPA state "
			   "machine");
		os_free(ctx);
		return -1;
	}
