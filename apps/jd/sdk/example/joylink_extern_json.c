...
joylink_dev_parse_ctrl(const char *pMsg, user_dev_status_t *userDev)
{
  ...
  if(NULL != pStreams){
    ...
    for( iCnt = 0; iCnt < iSize; iCnt++){
      ...
      if(!jl_platform_strcmp(USER_DATA_BRIGHTLED, pSId->valuestring)){
          if(pV->type == cJSON_String){
        userDev->BrightLED = jl_platform_atoi(pV->valuestring);
          }
          else if(pV->type == cJSON_Number){
              userDev->BrightLED_old = userDev->BrightLED;
              userDev->BrightLED = pV->valueint;
          }
          joylink_dev_user_data_set(USER_DATA_BRIGHTLED,userDev);
      }
      if(!jl_platform_strcmp(USER_DATA_COLOR, pSId->valuestring)){
          if(pV->type == cJSON_String){
        userDev->Color = jl_platform_atoi(pV->valuestring);
          }
          else if(pV->type == cJSON_Number){
              userDev->Color_old = userDev->Color;
              userDev->Color = pV->valueint;
          }
          joylink_dev_user_data_set(USER_DATA_COLOR,userDev);
      }
#endif
...
joylink_dev_package_info(const int retCode, user_dev_status_t *userDev)
{
  ...
  cJSON *data_BrightLED = cJSON_CreateObject();
  cJSON_AddItemToArray(arrary, data_BrightLED);
  cJSON_AddStringToObject(data_BrightLED, "stream_id", "BrightLED");
  jl_platform_memset(i2str, 0, sizeof(i2str));
  jl_platform_sprintf(i2str, "%d", userDev->BrightLED);
  cJSON_AddStringToObject(data_BrightLED, "current_value", i2str);

  cJSON *data_Color = cJSON_CreateObject();
  cJSON_AddItemToArray(arrary, data_Color);
  cJSON_AddStringToObject(data_Color, "stream_id", "Color");
  jl_platform_memset(i2str, 0, sizeof(i2str));
  jl_platform_sprintf(i2str, "%d", userDev->Color);
  cJSON_AddStringToObject(data_Color, "current_value", i2str);
#endif
