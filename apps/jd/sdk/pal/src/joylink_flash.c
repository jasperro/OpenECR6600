 ...
 * @brief:set data in jl_flash
 *
 * @param: key ：Guideposts for writing elements
 *  value ： What to write
 *  len ：the number of char elements in "value"
 * 
 * @returns: success 0, failed -1
 */
...
int32_t jl_flash_SetString(const char* key, const char* value, int32_t len)
{
    ...
    if (ret == 0) {
        jl_platform_printf("key = %s, fwrite success!\n\r", key);
    }
    else {
        jl_platform_printf("fwrite error!\n\r");
        ret = -1;
    }
    return ret;
...
int32_t jl_flash_GetString(const char* key, char* buff, int32_t len)
{
    ...
    if (read_len == 0) {
        jl_platform_printf("jl_flash_GetString, fread failed, key : %s, len = %d, read_len = 0\n\r", key, len);
    } else {
       jl_platform_printf("jl_flash_GetString, key : %s, read_len = %d\n\r", key, read_len);
    }
    return read_len;
#else
    return 0;
